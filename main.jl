...
function simulation_run()
    X_rec, U_rec, Σ_rec, X_true_rec, X_UKF_rec = simulate_nonlinear_MPC(nonlinear_problem,
                                                            x₀₀, Σ₀₀, T)
    ...
    return achieved_cost, achieved_est_err, X_rec, U_rec, Σ_rec, X_true_rec, UKF_est_err
end
...
function run_nonlinear_mpcs()
    ...
    for i in 1:num_simulations
        ...
        current_time = time()
        achieved_cost, achieved_est_err, X_rec, U_rec, Σ_rec, X_true_rec, UKF_est_err = simulation_run()
    ...
    return cost_rec, est_err_rec, x_rec, u_rec, x_true_rec, cov_rec, UKF_est_err_rec
end



function simulate_run_mpc()
    X_rec_mpc, U_rec_mpc, Σ_rec_mpc, X_true_rec_mpc, X_UKF_rec_mpc = simulate_mpc(linear_problem, x₀₀, Σ₀₀, T)
    ...
    return achieved_cost, achieved_est_err, X_rec_mpc, U_rec_mpc, Σ_rec_mpc, X_true_rec_mpc, UKF_est_err
end
...
function run_linear_mpcs()
    ...
    for i in 1:num_simulations
        current_time = time()
        achieved_cost, achieved_est_err, X_rec_mpc, U_rec_mpc, Σ_rec_mpc, X_true_rec_mpc, UKF_est_err = simulate_run_mpc()
    ...
    return cost_rec_mpc, est_err_rec_mpc, x_rec_mpc, u_rec_mpc, x_true_rec_mpc, cov_rec_mpc, UKF_est_err_rec_mpc
end
...
if RUN_SIMS
    ...
    cost_rec, est_err_rec, x_rec, u_rec, x_true_rec, cov_rec, UKF_est_err_rec = run_nonlinear_mpcs()
    println("Running linear MPC simulations...")
    cost_rec_mpc, est_err_rec_mpc, x_rec_mpc, u_rec_mpc, x_true_rec_mpc, cov_rec_mpc, UKF_est_err_rec_mpc = run_linear_mpcs()
    ...
    SAVE_DATA = true
    if SAVE_DATA
        @save "simulation_results100.jld2" cov_rec cost_rec est_err_rec UKF_est_err_rec cov_rec_mpc cost_rec_mpc est_err_rec_mpc UKF_est_err_rec_mpc
    end
end
