function simulate_nonlinear_MPC(nonlinear_problem, x₀₀, Σ₀₀, T)
    ...
    mhe_inst = MHE.MovingHorizonEstimator(state_dynamics, measurement_dynamics, W, V, 8, x₀₀, Σ₀₀)
    x_MHE₀₀ = copy(x₀₀)
    X_MHE_rec = [x_MHE₀₀ for _ in 1:T]
    ...
    for k in 1:T
        ...
        x_MHE₀₀, _ = MHE.update(x_MHE₀₀, nothing, u, y, mhe_inst)
    end
    return X_rec, U_rec, Σ_rec, X_true_rec, X_UKF_rec, X_MHE_rec
end
...
function simulate_mpc(lin_prob::MPC_Prob, x₀₀, Σ₀₀, T)
    ...
    return X_rec_mpc, U_rec_mpc, Σ_rec_mpc, X_true_rec_mpc, X_UKF_rec, X_MHE_rec
end
